...
- The frontend polls `/api/status/<job_id>` every 1.5s and shows a download
...
|--------|----------------------------|---------------------------------------|
| POST   | `/api/download`            | Body: `{"url": "..."}` → returns `job_id` |
| GET    | `/api/status/<job_id>`     | Returns job status/progress           |
| GET    | `/api/file/<job_id>`       | Downloads the finished file           |
| POST   | `/api/cleanup/<job_id>`    | Deletes the job's files from disk     |
